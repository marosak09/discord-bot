...
async def help(ctx):
    if ctx.invoked_subcommand is None:
        em = discord.Embed(title="Help", description="Help with categories", color=discord.Color.gold())
...
async def moderation(ctx):
    em = discord.Embed(title="Moderation Help", description="Help with moderation commands", color=discord.Color.blue())
...
@help.command()
async def setup(ctx):
    em = discord.Embed(title="Setup Help", description="Help with setup commands", color=discord.Color.blue())
...
@help.command()
async def fun(ctx):
    em = discord.Embed(title="Fun Help", description="Help with fun commands", color=discord.Color.blue())
...
@help.command()
async def other(ctx):
    em = discord.Embed(title="Other Help", description="Help with other commands", color=discord.Color.blue())
...
async def about(ctx):
    em = discord.Embed(title="About", description="Bot info", color=discord.colour.Color.yellow())
    em.set_author(name=name, icon_url=emico)
...
async def changelog(ctx):
    em = discord.Embed(title="Changelog", description=f"{name} v{version}", color=discord.colour.Color.yellow())
    em.set_author(name=name, icon_url=emico)
...
async def server_info(ctx):
    ...
    em = discord.Embed(title=f"Server Info - {guild.name}", color=discord.Color.blue())
...
async def reddit(ctx, sub):
    ...
    if nsfw and not ctx.channel.is_nsfw():
        ...
        em.set_footer(text=f"Created by: {redditPoster}, subreddit: {subreddit}, post: {postLink}")
        await ctx.send(embed=em)
...
async def warn_role(ctx, role: discord.Role):
    ...
    if warn_role_ref.get():
        ...
        if old_warn_role:
            await ctx.send(f"Replacing warn role from {old_warn_role.mention} to {role.mention}")
        else:
...
async def remove_warn_role(ctx, role: discord.Role):
    ...
    if second_warn_role_ref.get():
        ...
        if old_second_warn_role:
            await ctx.send(f"Replacing second warn role from {old_second_warn_role.mention} to {role.mention}")
        else:
        ...
        if warn_role_id and second_warn_role_id:
            ...
            if total_warns >= warn_role_threshold:
                try:
                    ...
                    await ctx.send(f"{member.mention} has been given the {role.name} role due to reaching {warn_role_threshold} warns.")
                    user_warns_ref.delete()
                    
                    role_to_remove = ctx.guild.get_role(second_warn_role_id)
                    if role_to_remove in member.roles:
                        await member.remove_roles(role_to_remove)
                        await ctx.send(f"{member.mention} has been removed from {role_to_remove.name} role.")
                except Exception as e:
...
async def log_room(ctx, channel: discord.TextChannel):
    global log_channel
    if log_channel:
        await ctx.send(f"Replacing log room from {log_channel.mention} to {channel.mention}")
        await log_channel.send("This channel has been unassigned as the log room.")
        await log_channel.send(embed=discord.Embed(description="Thank you!", color=discord.Color.green()))
    log_channel = channel
    save_config()
